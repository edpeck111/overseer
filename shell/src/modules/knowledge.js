...
export function mountKnowledge(root, store, ctx) {
  ...
  screen.append(tabbar, bannerBar, body);
  ...
  function paint() {
    ...
    // DISABLED banners — knowledge corpus is always synthetic until ZIM files are loaded
    if (local.hw) {
      bannerBar.appendChild(disabledBanner(
        "KNOWLEDGE BASE",
        "no ZIM files present — real corpus requires offline Wikipedia/Kiwix ZIM"
      ));
    }
