...
def reset_for_tests() -> None:
    ...
    reset_tables('comms_operator', 'comms_message', 'comms_board_post')
...
def register_operator(callsign: str) -> dict:
    ...
    try:
        db = get_db()
        db.execute(
            'INSERT OR IGNORE INTO comms_operator(callsign) VALUES(?)',
            (callsign,))
        db.commit()
    except Exception:
        pass  # DB not yet migrated is fine during imports
...
def send_message(from_cs: str, to_cs: str, subj: str, body: str, *, hops: int = 1) -> int:
    ...
    try:
        ...
        db.execute(
            ...
            '(id, from_cs, to_cs, subj, state, hops, when_ts, envelope_json)'
            ' VALUES(?,?,?,?,?,?,?,?)',
            (msg.id, from_cs, to_cs, subj, msg.state, hops,
             msg.when, _json.dumps(env.to_wire())))
        db.commit()
    except Exception:
        pass
    return msg.id
...
def mark_read(callsign: str, ids: list[int]) -> int:
    ...
    for mid in ids:
        ...
        if m and m.to_cs == callsign and m.state != "read":
            ...
            try:
                db = get_db()
                db.execute("UPDATE comms_message SET state='read' WHERE id=?", (mid,))
                db.commit()
            except Exception:
                pass
    return n
...
def post_to_board(callsign: str, board: str, subj: str, body: str) -> int:
    ...
    try:
        db = get_db()
        db.execute(
            'INSERT INTO comms_board_post(id, board, from_cs, subj, body, when_ts)'
            ' VALUES(?,?,?,?,?,?)',
            (p.id, board, callsign, subj, body, p.when))
        db.commit()
    except Exception:
        pass
    return p.id
