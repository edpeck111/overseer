...
export function mountPower(root, store, ctx) {
  ...
  // Inject DISABLED banner if hardware is synthetic; it sits above the tiles.
  hwStatus().then(h => {
    if (h && h._synthetic && h._synthetic.power) {
      screen.insertBefore(
        disabledBanner("POWER MONITOR", "set OVERSEER_POWER=ina226|shunt"),
        screen.firstChild,
      );
    }
  });
